function dynamic_local()
    % The parameters are defined based on Dunlap and Kenkre (1986)
    % System properties
    N = 101;            % Lattice size (odd to have a distinct center)
    center_site = (N+1)/2;
    V = 1;              % Nearest-neighbor hopping integral
    hbar = 1;           % Reduced Planck's constant (set to 1 as in paper)
    ...
    % Case 2: AC Field with "Magic Ratio" (Localized) -> First root of J0 ~ 2.405 
    ...
    grid on;
    % Replicate paper observation: Bounded MSD for Ratio=2.405
...
function dpsidt = hamiltonian_func(t, psi, N, V, E_mag, omega, hbar)
    % Constructing the Hamiltonian matrix H(t)
    
    % 1. We 1st define off-diagonal elements (Hopping V) 
    % H includes |m><m+1| and |m+1><m|
    e = ones(N, 1);
    H_kin = spdiags([V*e V*e], [-1 
    
    % 2. Diagonal elements (Electric Potential)
    % The paper Hamiltonian term is -eE(t)a * sum(m |m><m|)
